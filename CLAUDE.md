...
3. Run two waterfalls in parallel (`Promise.allSettled`), each with its own error flag. **A resolved `domain` is an unambiguous company match; the `company` name is not** (e.g. "Orthogonal" → the YC W26 company *and* the unrelated orthogonal.io). So the finders are **domain-first, and when a domain exists they search by domain ONLY** — returning nothing is better than returning a same-named company's people. Name + Coresignal steps run **only when no domain was resolved**.
   - **People (target 5):** *domain present* → ContactOut `people/search` (domain + title) → (domain only). *No domain* → ContactOut (company + title) → (company only) → Coresignal `employee_base/search/filter/preview` (company).
   - **Recruiters (target 3):** *domain present* → ContactOut (domain + 13 recruiter titles) only (none → empty, which is correct). *No domain* → ContactOut (company + recruiter titles) → Coresignal (company + title "Recruiter") → Coresignal (company only — founders hire directly).

**Alumni flow** (`components/AlumniFinder.tsx` → `app/api/alumni/route.ts`) — opt-in secondary action; the school is **not** asked for up front. Takes company + domain (from the search response) + school. Same domain-first rule: *domain present* → ContactOut (domain + education) only; *no domain* → ContactOut (company + education).
...
- **People + Recruiters** (each ContactOut step $0.05, Coresignal $0.021; run in parallel; best $0.05 when the first step hits): *domain present* → People ≤ $0.10 (domain+title → domain), Recruiters ≤ $0.05. *No domain* → People ≤ $0.121 (company+title → company → Coresignal), Recruiters ≤ $0.092. Domain-present is cheaper *and* more accurate (no name fallback).
